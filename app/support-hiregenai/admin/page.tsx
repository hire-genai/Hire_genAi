...
const STATUS_STYLES: Record<string, { bg: string, text: string, label: string }> = {
  ...
  "waiting": { bg: "bg-purple-500/20", text: "text-purple-400", label: "Awaiting Reply" },
...
export default function SupportAdminPage() {
  ...
  const [resolvedCounts, setResolvedCounts] = useState({ all: 0, feedback: 0, tickets: 0 })
  ...
  // Refresh tickets periodically (reduced frequency)
  useEffect(() => {
    const interval = setInterval(loadTickets, 60000) // Changed from 15s to 60s
  ...
  const handleScreenshotUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res = await fetch("/api/support/upload-screenshot", {
        method: "POST",
        body: formData
      })
  ...
  const handleSendReply = async () => {
    if (!selectedTicket || (!replyText.trim() && !replyScreenshot)) return
    ...
    try {
      const res = await fetch(`/api/admin/tickets/${selectedTicket.id}/messages`, {
        ...
        body: JSON.stringify({
          message: replyText.trim() || "(Image attached)",
      ...
      if (data.success) {
        ...
        // Removed loadTickets() call to prevent unnecessary API requests
      }
    } catch (error) {
  ...
  const handleResolveTicket = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Only refresh tickets after resolving (this is necessary to update status)
        loadTickets()
      }
    } catch (error) {
  ...
  const openTickets = tickets.filter(t => ["open", "in_progress", "waiting"].includes(t.status) && t.type !== "feedback")
  const allResolvedTickets = tickets.filter(t => ["resolved", "closed"].includes(t.status))
  const feedbackItems = tickets.filter(t => t.type === "feedback" && ["open", "in_progress", "waiting"].includes(t.status))
  
  const resolvedTickets = resolvedFilter === "all" 
    ? allResolvedTickets 
    : resolvedFilter === "feedback"
    ? allResolvedTickets.filter(t => t.type === "feedback")
    : allResolvedTickets.filter(t => t.type !== "feedback")

  useEffect(() => {
    const allResolved = tickets.filter(t => ["resolved", "closed"].includes(t.status))
    const feedbackCount = allResolved.filter(t => t.type === "feedback").length
    const ticketsCount = allResolved.filter(t => t.type !== "feedback").length
    setResolvedCounts({ all: allResolved.length, feedback: feedbackCount, tickets: ticketsCount })
  }, [tickets])
    ...
    <div className="min-h-screen bg-slate-900">
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-6 py-3 sm:py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 sm:gap-4">
              ...
              <button
                onClick={() => router.push("/admin-hiregenai")}
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 py-4 sm:py-6">
          ...
          <div className="flex sm:grid sm:grid-cols-5 gap-2 sm:gap-4 w-max sm:w-auto">
            ...
            <div className="bg-slate-800 rounded-lg sm:rounded-xl p-2 sm:p-4 border border-slate-700 min-w-[80px] sm:min-w-0">
              <div className="flex flex-col sm:flex-row items-center sm:items-center gap-1 sm:gap-3">
                ...
                <div className="text-center sm:text-left">
                  <p className="text-lg sm:text-2xl font-bold text-white">{allResolvedTickets.length}</p>
              ...
              <div className="flex flex-col sm:flex-row items-center sm:items-center gap-1 sm:gap-3">
                ...
                <div className="text-center sm:text-left">
                  <p className="text-lg sm:text-2xl font-bold text-white">
                    {tickets.filter(t => t.priority === "urgent" && t.status === "open" && t.type !== "feedback").length}
                  </p>
        ...
        <div className="overflow-x-auto scrollbar-hide mb-4 sm:mb-6 -mx-3 px-3 sm:mx-0 sm:px-0">
          <div className="flex gap-1 bg-slate-800 p-1 rounded-lg w-max sm:w-fit border border-slate-700">
            <button
              ...
              className={`flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 rounded-md text-xs sm:text-sm font-medium transition-colors whitespace-nowrap ${
                activeTab === "open" ? "bg-emerald-600 text-white" : "text-slate-400 hover:text-white"
              }`}
            >
              ...
              {openTickets.length > 0 && (
                <span className={`px-1.5 sm:px-2 py-0.5 rounded-full text-[10px] sm:text-xs ${activeTab === "open" ? "bg-emerald-500" : "bg-slate-700"}`}>
                  {openTickets.length}
                </span>
              )}
            </button>
            <button
              ...
              className={`flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 rounded-md text-xs sm:text-sm font-medium transition-colors whitespace-nowrap ${
                activeTab === "resolved" ? "bg-emerald-600 text-white" : "text-slate-400 hover:text-white"
              }`}
            >
              <CheckCircle className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
              Resolved
              {allResolvedTickets.length > 0 && (
                <span className={`px-1.5 sm:px-2 py-0.5 rounded-full text-[10px] sm:text-xs ${activeTab === "resolved" ? "bg-emerald-500" : "bg-slate-700"}`}>
                  {allResolvedTickets.length}
                </span>
              )}
            </button>
            <button
              ...
              className={`flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 rounded-md text-xs sm:text-sm font-medium transition-colors whitespace-nowrap ${
                activeTab === "feedback" ? "bg-amber-600 text-white" : "text-slate-400 hover:text-white"
              }`}
            >
              <Lightbulb className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
              Feedback
              {feedbackItems.length > 0 && (
                <span className={`px-1.5 sm:px-2 py-0.5 rounded-full text-[10px] sm:text-xs ${activeTab === "feedback" ? "bg-amber-500" : "bg-slate-700"}`}>
                ...
                <div className="mt-3 flex gap-2">
                  <button
                    ...
                    className={`px-2 py-1 text-xs rounded transition-colors ${
                      resolvedFilter === "all" ? "bg-slate-600 text-white" : "bg-slate-700 text-slate-300 hover:bg-slate-600"
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`px-2 py-1 text-xs rounded transition-colors ${
                      resolvedFilter === "feedback" ? "bg-amber-500 text-white" : "bg-amber-500/20 text-amber-400 hover:bg-amber-500/30"
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`px-2 py-1 text-xs rounded transition-colors ${
                      resolvedFilter === "tickets" ? "bg-emerald-500 text-white" : "bg-emerald-500/20 text-emerald-400 hover:bg-emerald-500/30"
                    }`}
                  >
                  ...
                  {(activeTab === "open" ? openTickets : activeTab === "resolved" ? resolvedTickets : feedbackItems).map(ticket => (
                    <div
                      ...
                      className={`p-4 cursor-pointer transition-colors ${
                        selectedTicket?.id === ticket.id ? "bg-slate-700" : "hover:bg-slate-700/50"
                      }`}
                    >
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className="text-xs text-slate-400">{ticket.company_name}</span>
                      </div>
                      <div className="flex items-center gap-2 mt-1">
                  ...
                  {selectedTicket.messages?.map((msg, idx) => (
                    ...
                    >
                      ...
                      >
                        ...
                        <div className={`text-xs mt-1 ${msg.sender_type === "support" ? "text-emerald-200" : "text-slate-400"}`}>
                  ...
                  <div className="p-2 border-t border-slate-700">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="p-1.5 text-slate-400 hover:text-white rounded transition-colors disabled:opacity-50"
                      ...
                      <button
                        ...
                        disabled={(!replyText.trim() && !replyScreenshot) || isSending}
