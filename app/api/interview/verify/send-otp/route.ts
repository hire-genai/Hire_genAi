...
export async function POST(req: Request) {
  try {
    ...
    // Get candidate email from application via candidates table
    const query = `
      SELECT 
        c.email, 
        c.first_name, 
    ...
    // Send OTP email
    const candidateName = application.first_name || application.full_name || 'Candidate'
        ...
        <div style="max-width: 480px; margin: 0 auto; background: white; border-radius: 12px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); overflow: hidden;">
          ...
          <div style="background: #f8fafc; padding: 16px 32px; text-align: center; border-top: 1px solid #e5e7eb;">
            <p style="color: #9ca3af; font-size: 12px; margin: 0;">
              &copy; ${new Date().getFullYear()} HireGenAI. All rights reserved.
            </p>
          </div>
        </div>
      </body>
