  ...
  const runQualityGate = useCallback(async (imageData: string): Promise<{ passed: boolean; error?: string; detection?: any }> => {
    ...
    try {
      ...
      if (confidence < MIN_CONFIDENCE) {
        ...
        return { passed: false, error: `Face not clear (${(confidence * 100).toFixed(0)}% confidence). Please face the camera directly.` }
      }
      ...
      if (horizontalDeviation > MAX_CENTER_DEVIATION) {
        ...
        return { passed: false, error: 'Face not centered. Please align your face with the oval guide.' }
      }
                ...
                {isCameraOpen && !capturedPhoto && (
                  <div className="flex flex-col items-center">
                    ...
                    <div className={`w-full max-w-xs mb-3 p-2 rounded-lg text-center text-sm font-medium ${
                      ...
                      {!cameraStabilizing && faceStatus === 'not_centered' && 'Center your face in the oval'}
