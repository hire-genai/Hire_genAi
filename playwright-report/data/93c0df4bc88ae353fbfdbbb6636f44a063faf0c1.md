...
- Name: 20-contact.spec.ts >> Contact — Footer >> 8.4 TrustScore badge shown
- Location: tests\e2e\20-contact.spec.ts:386:7
...
```ts
  288 |     await expect(page.getByText(/Thank you for reaching out/i)).toBeVisible()
  289 |   })
  290 | 
  291 |   test('6.3 "Return to Home" button visible', async ({ page }) => {
  292 |     await submitForm(page)
  293 |     await expect(page.getByRole('button', { name: /Return to Home/i })).toBeVisible()
  294 |   })
  295 | 
